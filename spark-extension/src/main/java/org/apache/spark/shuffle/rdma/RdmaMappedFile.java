...
public class RdmaMappedFile {
  ...
  public long allMmapFileLength() {
    long result = 0;
    for (RdmaFileMapping rdmaFileMapping : rdmaFileMappings) {
      result += rdmaFileMapping.length;
    }
    return result ;
  }
  ...
  public ByteBuffer getByteBufferForPartition(int startPartitionId,
                                              int endPartitionId) throws IOException {
    RdmaBlockLocation rdmaBlockLocation =
            rdmaMapTaskOutput.getRdmaBlockLocation(startPartitionId, endPartitionId);
    return (rdmaBlockLocation.length() == 0) ? null :
            getByteBuffer(rdmaBlockLocation.address(), rdmaBlockLocation.length());
  }
