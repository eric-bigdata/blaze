...
public class RdmaChannel {
    ...
    private void setupCommon() throws IOException {
        ...
        // logger.info("recvDepth " + recvDepth + "  sendDepth" + sendDepth);
    ...
    void connect(InetSocketAddress socketAddress) throws IOException {
        ...
        try {
            ...
            throw new IOException("resolveAddr() failed: " + e);
        }

        try {
            cmId.resolveRoute(resolvePathTimeout);
        } catch (IOException e) {
            throw new IOException("resolveRoute() failed: " + e);
        }
    ...
    public InetSocketAddress getSourceSocketAddress() throws IOException {
    ...
    void finalizeConnection() {
        ...
        synchronized (rdmaChannelState) {
            rdmaChannelState.notifyAll();
        }
    }

    private void processRdmaCmEvent(int expectedEvent, int timeout) throws IOException {
        RdmaCmEvent event = null;
        if (eventChannel != null) {
            event = eventChannel.getCmEvent(timeout);
        }
    ...
    private void postZeroSizeRecvWrs(int count) throws IOException {
        if (isError() || isStopped.get() || recvDepth == 0) {
            return;
        }
        ...
        while (true) {
            ...
            if (res < 0) {
                ...
                for (int i = 0; i < res; i++) {
                    ...
                    if (!wcSuccess && !isError() && !isStopped.get()) {
                        setRdmaChannelState(RdmaChannelState.ERROR);
    ...
    void stop() throws InterruptedException, IOException {
        if (!isStopped.getAndSet(true)) {
            logger.info("Stopping RdmaChannel " + this + this.cmId.getSource());
    ...
    boolean isConnected() {
        return rdmaChannelState.get() == RdmaChannelState.CONNECTED.ordinal();
    }

    boolean isError() {
        return rdmaChannelState.get() == RdmaChannelState.ERROR.ordinal();
    }
