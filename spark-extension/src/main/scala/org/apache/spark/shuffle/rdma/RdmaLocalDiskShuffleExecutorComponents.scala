...
class RdmaLocalDiskShuffleExecutorComponents(sparkConf: SparkConf)
  extends LocalDiskShuffleExecutorComponents(sparkConf) with Logging{

  private var blockResolver: RdmaShuffleBlockResolver = _

  override def initializeExecutor(appId: String, execId: String,
                                  extraConfigs: util.Map[String, String]): Unit = {
    val rdmaShuffleManager = SparkEnv.get.shuffleManager.asInstanceOf[RdmaShuffleManager]
    rdmaShuffleManager.startRdmaNodeIfMissing()
    blockResolver = rdmaShuffleManager.shuffleBlockResolver
  }
