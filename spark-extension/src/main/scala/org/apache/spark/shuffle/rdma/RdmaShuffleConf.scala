...
object SparkVersionSupport {
  ...
  if (majorVersion != 3) {
    throw new IllegalArgumentException("SparkRDMA only supports Spark versions 3.x")
  }
...
class RdmaShuffleConf(conf: SparkConf) extends Logging{
  ...
  //
  lazy val recvQueueDepth: Int = getRdmaConfIntInRange("recvQueueDepth", 256, 128, 65535)
