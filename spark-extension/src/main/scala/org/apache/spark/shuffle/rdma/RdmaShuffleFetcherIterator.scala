...
private[spark] final class RdmaShuffleFetcherIterator(
    ...
    mapIdToBlockIndex: Map[Long, Integer])
    ...
    val listener = new RdmaCompletionListener {
      override def onSuccess(paramBuf: ByteBuffer): Unit = {
        ...
        logTrace(s"Got remote block(s): of size ${pendingFetch.totalLength} " +
          ...
          s"after ${Utils.msDurationToString(System.currentTimeMillis() - startRemoteFetchTime)}")
      }
          ...
          case blockIdSeq =>
            ...
            val executorRdmaReadBlockLocationListener = new RdmaCompletionListener {
              override def onSuccess(buf: ByteBuffer): Unit = {
                ...
                logTrace(s"RDMA read ${shuffleBlocksCount} block locations " +
                  ...
                  s"${Utils.msDurationToString(System.currentTimeMillis() - startTime)}" +
                  s" total length ${totalLength}")
            ...
            shuffleBlocks.zipWithIndex.foreach {
              case (shuffleBlock, i) =>
                ...
                val mapIdOffset =
                  mapIdToBlockIndex.get(shuffleBlock.mapId).get * RdmaMapTaskOutput.MAP_ENTRY_SIZE
                val reduceIdOffset = mapTaskOutput.getLong(mapIdOffset.toInt) +
                  shuffleBlock.reduceId * RdmaMapTaskOutput.ENTRY_SIZE
                rAddresses(i) = reduceIdOffset
                rKeys(i) = mapTaskOutput.getInt(mapIdOffset.toInt + 8)
            }
            try {
  ...
  private[this] def initialize(): Unit = {
    ...
    if (endMapIndex != Int.MaxValue) {
      ...
      for (partitionId <- startPartition until endPartition) {
        rdmaShuffleManager.shuffleBlockResolver.
          getLocalRdmaPartition(shuffleId, partitionId, startMapIndex, endMapIndex).foreach{
          case in: InputStream =>
      ...
      for (partitionId <- startPartition until endPartition) {
        ...
        rdmaShuffleManager.shuffleBlockResolver.
          getLocalRdmaPartition(shuffleId, partitionId).foreach{
          case in: InputStream =>
            shuffleMetrics.incLocalBlocksFetched(1)
            shuffleMetrics.incLocalBytesRead(in.available())
            resultsQueue.put(SuccessFetchResult(partitionId, localBlockManagerId, in))
  ...
  override def next(): InputStream = {
    ...
    result match {
      ...
      case FailureFetchResult(partitionId, blockManagerId, e) =>
        throw new FetchFailedException(blockManagerId, shuffleId,
          0, 0, partitionId, e)
