...
class RdmaMapTaskOutput private[shuffle](
  ...
  private[shuffle] def getNumPartitions: Int = lastPartitionId - startPartitionId + 1
  private[shuffle] def size: Int = getNumPartitions * ENTRY_SIZE
  ...
  private[shuffle] def getRdmaBlockLocation(requestedId: Int) = {
  ...
  private[shuffle] def getRdmaBlockLocation(startPartition: Int, endPartitionId: Int) = {
    if (endPartitionId < startPartition || endPartitionId > lastPartitionId) {
      throw new IndexOutOfBoundsException("PartitionId " + endPartitionId + " is out of range (" +
        startPartition + "-" + lastPartitionId + ")")
    }
    RdmaBlockLocation(
      byteBuffer.getLong((endPartitionId - startPartition) * ENTRY_SIZE),
      byteBuffer.getInt((endPartitionId - startPartition) * ENTRY_SIZE + 8),
      byteBuffer.getInt((endPartitionId - startPartition) * ENTRY_SIZE + 8 + 4))
  }

  private[shuffle] def getByteBuffer(firstRequestedId: Int, lastRequestedId: Int) = {
  ...
  private[shuffle] def put(requestedId: Int, address: Long, length: Int, mKey: Int) = {
    if (requestedId < startPartitionId || requestedId > lastPartitionId) {
      throw new IndexOutOfBoundsException("PartitionId " + requestedId + " is out of range (" +
        startPartitionId + "-" + lastPartitionId + ")")
    }
